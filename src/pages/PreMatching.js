  ...
  <div className="pre-select-mode-card">
    ...
    <div className="pre-card-content">
      ...
      <div className="pre-select-mode-buttons">
        <button
          className="pre-select-mode-btn"
          onClick={() => { setSelectedMatch('자동'); handleStartMatching(); }}
        >
          자동 매칭
        </button>
        <button
          className="pre-select-mode-btn"
          onClick={() => { setSelectedMatch('수동'); setStep('filter'); }}
        >
          수동 매칭
        </button>
      </div>
      ...
      {step === 'filter' && (
        <div className="pre-matching-content">
          <div className="pre-filter-bar">
            <div className="pre-filter-items">
            ...
            <button className="pre-filter-apply-btn" onClick={handleFilterApply}>필터적용</button>
          </div>
          ...
          {filtered && (
            <div className="pre-card-grid">
              {filteredGameRooms.map((room, i) => (
                <div className="pre-card" key={room.id}>
                  <div className="pre-card-thumbnail">
                    <div className="pre-thumbnail-text">1/5</div>
                    <button className={`pre-heart-button ${liked[i] ? 'liked' : ''}`} onClick={() => toggleLike(i)}>❤︎</button>
                  </div>
